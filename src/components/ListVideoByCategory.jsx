...
const ListVideoByCategory = ({ q }) => {
  ...
  const handleNextSearch = async () => {
    ...
    if (q) {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_URL_API}/search?part=snippet&maxResults=30&pageToken=${pageToken}&q=${q}&key=${process.env.NEXT_PUBLIC_KEY}`
      );
  ...
  useEffect(() => {
    const handleSearch = async () => {
      ...
      if (q) {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_URL_API}/search?part=snippet&maxResults=30&q=${q}&key=${process.env.NEXT_PUBLIC_KEY}`
        );
...
export default ListVideoByCategory;
